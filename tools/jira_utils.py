...
if not JIRA_DOMAIN:
    raise ValueError("JIRA_DOMAIN environment variable is not set")
if not JIRA_DOMAIN.startswith("https://"):
    JIRA_DOMAIN = f"https://{JIRA_DOMAIN}"
if not re.match(r'^https://[a-zA-Z0-9-]+\.atlassian\.net$', JIRA_DOMAIN):
    raise ValueError(f"Invalid JIRA_DOMAIN format: {JIRA_DOMAIN}")
...
def get_user_account_id(user_email: str):
    ...
    try:
        ...
        print(f"[ERROR] DNS resolution failed for {JIRA_DOMAIN}: {e}")
        raise
...
def fetch_user_tasks(project_key: str, user_email: str):
    ...
    try:
        ...
        print(f"[ERROR] DNS resolution failed for {JIRA_DOMAIN}. Check JIRA_DOMAIN configuration: {e}")
        return []
    except requests.exceptions.HTTPError as e:
...
def reassign_task(task_key: str, new_assignee_account_id: str):
    ...
    url = f"{JIRA_DOMAIN}/rest/api/3/issue/{task_key}/assignee"
